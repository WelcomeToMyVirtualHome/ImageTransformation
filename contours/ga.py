...
def new_generation(parents, size, number_of_best):
    ...
    while True:
        ...
        for extra1,extra2 in zip(img1,img2):      
            ...
            angle_1 = int_to_bitfield(int(extra1.angle))
            angle_2 = int_to_bitfield(int(extra2.angle))
            ...
            chunks.append(Chunk(extra1.img,[scale_chy,scale_chx],bitfield_to_int(angle_ch),[bitfield_to_int(pos_chy)%H,bitfield_to_int(pos_chx)%W]))
...
generation_size = 500
number_of_best = 2
generation = [[Chunk(img,[1,1],0,[np.random.randint(low=0,high=H),np.random.randint(low=0,high=W)]) for img in extracted] for i in range(generation_size)]
