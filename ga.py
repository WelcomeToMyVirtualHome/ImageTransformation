...
print("Score: identity={:.2f}, empty={:.2f}".format(ssim(image, image, multichannel=True),
                                                    ssim(image, np.zeros(image.shape, dtype=np.uint8),
                                                         multichannel=True)))
...
def new_generation(parents, size, number_of_best):
    ...
    while True:
        ...
        for extra1, extra2 in zip(img1, img2):
            ...
            chunks.append(Chunk(extra1.img, [scale_y, scale_x], angle,
                                [bitfield_to_int(pos_chy) % H, bitfield_to_int(pos_chx) % W]))
...
generation_size = 10
number_of_best = 2
generation = [[Chunk(img, [1, 1], np.random.randint(low=-180, high=180),
                     [np.random.randint(low=0, high=H), np.random.randint(low=0, high=W)]) for img in extracted] for i
              in range(generation_size)]
